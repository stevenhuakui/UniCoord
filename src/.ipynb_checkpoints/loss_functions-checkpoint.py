...
def cal_diffusion_loss(latent_dist, diffuse_cont_dims,
    ...
    diffused_latent_dist = latent_dist
    diffused_latent_dist['cont'][0] = torch.mm(neighbor_prob, diffused_latent_dist['cont'][0])
    diffused_latent_dist['cont'][1] = torch.mm(neighbor_prob, diffused_latent_dist['cont'][1])
    # after diffusion, use diffused latent distribution to do sampling and decoding,
    # calculate reconstruction error
    neighbor_recon_data = model.decode(model.reparameterize(diffused_latent_dist))
...
def cal_reg_loss_sign(latent_dist_cont, label, cont_supervise_dict, use_cuda, factor=1.0):
    """
    ...
    reg_loss_sign = 0
    for l in cont_supervise_dict:
        # the lth latent dim is supervised by the cont_supervise_dict[l]th label
        alpha = mean[:,l]
        attribute = label[:,cont_supervise_dict[l]]
        ...
        reg_loss_sign += sign_loss
    return reg_loss_sign

def cal_cross_entropy(latent_dist_disc, label,disc_supervise_dict, use_cuda):
    """
    Computes the cross entropy loss given the latent disc dims and labels
    ...
    """
    # if disc_supervise_dict is not set, all cont dims will be supervised by the first label
    if not disc_supervise_dict:
        disc_supervise_dict = {i:0 for i in range(0,len(latent_dist_disc))}
    cross_entropy = 0
    for l in disc_supervise_dict:
        alpha = latent_dist_disc[l]
        attribute = label[:,disc_supervise_dict[l]]
