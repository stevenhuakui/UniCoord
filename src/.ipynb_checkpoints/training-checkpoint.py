...
#             else:
    ...
    def _loss_function(self, data, recon_data, latent_dist,label):
        ...
        if self.model.is_continuous:
            ...
            loss_dict['cont_capacity_loss'] = cont_capacity_loss
        ...
        if self.model.is_discrete:
            ...
            loss_dict['disc_capacity_loss'] = disc_capacity_loss
        ...
        if self.diffusion_lambda >0:
            diffusion_loss = cal_diffusion_loss(latent_dist, self.diffuse_cont_dims, data, self.model,
                                    self.use_cuda, self.sigma, self.random_walk_step)
        ...
        if self.discrete_supervised_lambda > 0:
            ...
            #F.cross_entropy(latent_dist['disc'][0],label[:,0].type(torch.int64).cuda())
